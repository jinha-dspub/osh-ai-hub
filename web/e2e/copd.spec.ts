...
test("COPD introduction explains the demo without inactive search controls", async ({
  ...
  await expect(
    page.getByRole("heading", { name: "이렇게 활용하세요" }),
  ).toBeVisible();
  await expect(
    page.getByRole("heading", {
      name: "공개 판정문에 검색 가능한 구조를 더했습니다",
    }),
  ).toBeVisible();
  await expect(
  ...
  await expect(
    page.getByRole("heading", { name: "데이터 미리보기" }),
  ).toBeVisible();
  await expect(page.locator(".copd-sheet tbody tr")).toHaveCount(3);
  await page.screenshot({
    path: `test-results/copd-preview-${page.viewportSize()?.width}.png`,
    fullPage: true,
  });
  await page.getByRole("button", { name: "품질·한계", exact: true }).click();
  await expect(page.getByText("1,919건", {exact:true})).toBeVisible();
  ...
  await page.screenshot({
    path: `test-results/copd-intro-${page.viewportSize()?.width}.png`,
    fullPage: true,
  });
  expect(
    await page.evaluate(
