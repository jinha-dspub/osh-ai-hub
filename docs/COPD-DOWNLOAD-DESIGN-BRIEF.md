...
사용자 요청에 따라 실제 cases.csv의 파일 순서 첫 3행에서 청구 연도·판정·AI 표준 직종·AI 유해인자만 공개 미리보기에 표시한다. 사건번호·개인정보·판정문·요약은 내보내지 않는다. 원본 전체를 web 번들에 넣지 않고 선택한 12개 셀만 별도 상수로 보관한다. ‘DEMO 미리보기 · 실제 자료 3행’으로 실제 자료 일부임을 밝히고 AI 가공 열을 구분한다. Excel처럼 A–D 열 표시와 행 번호·격자선을 사용하며 모바일은 표 영역만 가로 스크롤한다. 읽기 전용이고 전체 검색은 DEMO 링크로 이동한다. 원본 데이터와 접근 권한은 변경하지 않는다.
